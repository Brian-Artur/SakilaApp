...
const renderFilms = () => {
    ...
    <div class="mb-3 d-flex align-items-center flex-wrap gap-2">
      ...
      <span class="badge bg-warning text-dark ms-2">${filteredFilms.length} resultados</span>
    </div>
  `;
    ...
    <table class="table table-striped table-hover m-0 text-center">
      <thead class="table-danger align-middle text-center text-white">
        <tr>
          ${columnNames.map(col => `<th class="bg-danger text-white">${col.replace('_', ' ').toUpperCase()}</th>`).join('')}
        </tr>
      </thead>
      <tbody class="table-warning bg-opacity-75">
